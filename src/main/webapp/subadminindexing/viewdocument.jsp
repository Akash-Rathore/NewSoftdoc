...
<head>
            ...
            <div class="row clearfix">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        <div class="header">
                            <h2>
                                <a href="documents.jsp?id=<%=request.getParameter("id")%>"  class="btn btn-primary">Back</a> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;       Document
                            </h2>
                        </div>
                     ...
                     <form   class="form-horizontal form-submit">
                        ...
                        <div class="body">
                            ...
                            <div class="row">
                             <%
                                    try(Connection  connect=IndexingDBConnection.openConnection();PreparedStatement preparedStatement=connect.prepareStatement("SELECT attachment_no , document_info.id as id , count(attachment_no)as count , customeradmincode ,customer_name,customer_code,invoice_date,date_of_joining,employee_code,employee_name,invoice_number,vendor_name,vendor_code,document_type,document_no,sum(size) as size, file_no, company_name, department, upload_date,documentType1,documentType2,documentType3,documentType4,documentType5,documentType6 FROM customer_details, document_info WHERE document_info.company_id = customer_details.id  AND  document_info.file_no =(select file_no from document_info where id='"+request.getParameter("id")+"') AND document_info.document_no =(select document_no from document_info where id='"+request.getParameter("id")+"') AND document_info.status = 'indexing'");)
                                    {
                           ...
                           %>
                                ...
                                <div class="form-group">
                                    <label for="NameSurname" class="col-sm-2 control-label">Department:-</label>
                                       <div class="col-sm-10">
                                          <input type='text' value="<%=resultset.getString("department")%>" name="companyname" class="form-control show-tick" readonly="readonly">
                                      </div>
                                </div>
                                <div class="form-group">
                                    <label for="NameSurname" class="col-sm-2 control-label">Document Type:-</label>
                                    <div class="col-sm-10">
                                        <input type='text' value="<%=resultset.getString("document_type")%>" name="companyname" class="form-control show-tick" readonly="readonly">
                                    </div>
                                </div>
                              </div>  
                             <div class="row">
                             ...
                             <%} %>
                                ...
                                <div class="form-group">
                                   <label for="NameSurname" class="col-sm-2 control-label">Attachment Upload</label>
                                      <div class="col-sm-8 table-responsive add_attchment">
                         ...
                         <div class="row">
                            ...
                            <div class="table-responsive">
                                ...
                                <tbody>
                             <%
                             try(Connection  connect=IndexingDBConnection.openConnection();PreparedStatement preparedStatement=connect.prepareStatement("SELECT * FROM `document_info` WHERE document_no =(select document_no from document_info where id='"+request.getParameter("id")+"') AND status ='indexing' and file_no=(select file_no from document_info where id='"+request.getParameter("id")+"')");)
                             {
                                ResultSet resultset=preparedStatement.executeQuery();
                                int i=1;
                                while(resultset.next())
                                {
                                     %>
                                       <tr>
                                            <td><a href="<%=resultset.getString("file")%>"  onclick="onhit('<%=resultset.getString("id")%>')"><%=resultset.getString("attachment_no")%></a></td>
                                            <%
                                    ...
                                    %>
                                           <td><%=newDateString%></td>
                                             <td>
                                                 <button type="button" class="btn btn-primary delete" onclick="myFunction(<%=i%>)" data-file='<%=resultset.getString("file")%>' data-what='<%=resultset.getString("document_no")%>' data-id="<%=resultset.getInt("id")%>">Delete</button>
                                             </td>
   ...
   function statuschange(statuschnage) {
    ...
    function myFunction(count) {
    	...
    	if(count == 1)
        {
    		 //console.log(count +"akash rathore");
    		 error('You have to upload another attachment to delete previous attachment.There should be atleast one attachment required');
        }
    	else if(count >1)
        {
    		 console.log(count);
   ...
   function onhit(id) {
   $.ajax({
       url:"../indexingprocess?action=attachmenthit",
       type:"POST",
       dataType:"json",
       data:{"id":id},
