		...
		<div class="collapse navbar-collapse" id="navbar-collapse" style="background:#fff;">
	     ...
	     <div class="container-fluid" style="height: 38px; background: #6c757d;">
	          ...
	          <div style="position:relative;padding-left:242px;">
                  <input type="text" name="" id="input_css" value="" style="display:none;position:inherit ; top:-9px ;width: 400px !important; border: 2px solid #6c757d; border-radius: 25px;padding-left:10px; " placeholder="Search in Softdoc Cloud" />
...
<!--              <a href="#"  style="text-decoration:none;color:#fff;padding-left:309px;"><span style="font-size:32px;font-weight:bold;">+</span></a>
 -->              <a href="javascript:void(0);"  class="delete-all-document access_level_hide" style="text-decoration:none;color:#fff;padding-left:148px;"><span style="font-size:32px;font-weight:bold;"><i class="material-icons">delete</i></span></a>
                        ...
                        <div class="body">
                           ...
                           <form id="delete-folder">
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover" id="viewfolder">
                                <thead>
                                    <tr>
                                        <th>
                                            <input type='checkbox' onClick="toggle(this)" name='delete_folder' value='' id="delete"/><label for="delete"></label>
							            </th>
                                        <th>Document No.</th>
                                        <th>Document Type</th>
                                        <th></th>
	...
	<!-- Edit Folder end  -->
     ...
     <div class="modal fade" id="edit_folder" tabindex="-1" role="dialog"
		...
		<div class="modal-dialog  modal-lg" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title"  id="exampleModalLabel" style="text-align:center;text-decoration: underline;">Edit Folder</h5>
					<button type="button" class="close" data-dismiss="modal"
						aria-label="Close">
						<span aria-hidden="true">&times;</span>
					</button>
  ...
  <div class="modal-dialog" role="document">
   ...
   /* $(document).on('click' , '.delete-all-document' , function(event){
	   event.preventDefault();
	   
       
       var checked = new Array();//folderdocumentType
       $("#viewfolder input[type=checkbox]:checked").each(function () {
	    ...
	    Swal.fire({
           ...
           confirmButtonText: 'Yes'
   		  ...
   		  if (result.value) {
   		        ...
   		        $('#loader').show();
   				$.ajax({
         	            url: "../clientprocess?action=emptyTrashdocument",
         	            type: "post",
         	            dataType:"json",
   ...
   $(document).on('click' , '.start-folder' , function(event){

	   
	   Swal.fire({
	   		title: 'Are you sure?',
	           text: "Star DocumentType",
	           icon: 'warning',
	           showCancelButton: true,
	           cancelButtonText: 'No',
	           confirmButtonColor: '#3085d6',
    ...
    function uploadurl(){
       var url = window.location.href;
  	   var res = url.split("?");
  	   var  document_info_id = res[1];
       window.location.href='folderupload.jsp?'+document_info_id+'';
    }
 ...
 $(document).on('click', '.delete-folder-documentType' , function(){
 	...
 	Swal.fire({
 		 ...
 		 title: 'Are you sure?',
         text: "The Documents including all attachments will be moved to Trash for a period of 30 days, after which they will be permanently deleted",
         icon: 'warning',
         showCancelButton: true,
         cancelButtonText: 'No',
         confirmButtonColor: '#3085d6',
         ...
         confirmButtonText: 'Yes'
 		  ...
 		  if (result.value) {
 			   ...
 			   $('#loader').show();
 				$.ajax({
       	            url: "../clientprocess?action=trashDocumenttype",
       	            type: "post",
       	            dataType:"json",
   ...
   $(".addmore").on('click' , function(){
    ...
    html+= '<td><input type="text"  name="file_user_name['+i+']" placeholder="Search User Name"  class="form-control show-tick search-suggestion" value=""><a href="javascript:void(0);" style="text-decoration: none;color:#4c4a4a;"><div class="search-box" style="position:fixed;width:33.54%;z-index:9999;background:#fff;bottom:-57px;left:40px;"></div></a></td>';
    html+= '<td><select class="form-control show-tick" name="file_access_level['+i+']"><option value="">Access Level</option><option value="Read">Read</option><option value="Write">Write</option></select></td>';
   ...
   function toggle(source) {
    ...
    $(document).ready(function() {
       ...
       $("#viewfolder").dataTable( {
 	        ...
 	        "sort": "position",
 	        "ajax":{
  	   	       url:"../clientprocess?action=folderDocumentType",
  	  	       type:"POST",
  	  	       dataType:"json",
  	  	       data:{"document_info_id":document_info_id},
 	        },
