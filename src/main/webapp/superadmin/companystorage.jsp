                                ...
                                <table class="table  dashboard-task-infos display nowrap" id="mytable">
                                 ...
                                 <%
                                	try(Connection  connect=DBConnection.openConnection();
                                			PreparedStatement preparedStatement=connect.prepareStatement("SELECT SUM(ROUND(size,2))AS size , client_id , department , file_no , status , DATE_FORMAT(upload_date,'%d %b %Y')as upload_date FROM document_info  WHERE company_id = '"+request.getParameter("id")+"' and document_no is not null GROUP BY file_no");
                                            )
                                   {
                                		  int i=1;
