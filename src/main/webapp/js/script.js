...
$(document).ready(function () {
    $.ajax({
        url: 'Webservice_public/chat_default_response',
        dataType: 'json',
        success: function (res) {
            response_chat = res.response;
        },
    });
});
...
function chat_send(element) {
    ...
    if (your_msg) {
        ...
        $('#custom-chat .body #last').before(`<div class="sent">${your_msg}</div>`);
        ...
        if (localStorage.getItem('startChat') === null) {
            setTimeout(function () {
                $('#custom-chat .body #last').before(`<div class="received">${response_chat}</div>`);
                document.getElementById('last').scrollIntoView();
            }, 1500);
