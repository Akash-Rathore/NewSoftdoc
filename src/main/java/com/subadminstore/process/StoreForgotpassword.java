...
//@WebServlet(name = "/StoreForgotpassword", urlPatterns = { "" })
public class StoreForgotpassword extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    
    public StoreForgotpassword() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			...
			try{
				 ...
				 PreparedStatement preparedStatement = connection.prepareStatement("SELECT * FROM `softdoc_staff` WHERE email='" + email + "' AND role='store'");
				 ResultSet resultset = preparedStatement.executeQuery();
			     if(resultset.next())
			     {
			    	    
			    		 new com.subadminstore.email.EmailClient().email("Password recovery for your Softdoc Account",
