...
//@WebServlet(name = "/QualityForgotpassword", urlPatterns = { "/qualityforgot" })
public class QualityForgotpassword extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    
    public QualityForgotpassword() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			...
			try{
				 ...
				 PreparedStatement preparedStatement = connection.prepareStatement("SELECT * FROM `softdoc_staff` WHERE email='" + email + "' AND role='quality'");
				 ResultSet resultset = preparedStatement.executeQuery();
			     if(resultset.next())
			     {
			    	    
			    		 new com.subadminquality.email.EmailClient().email("Password recovery for your Softdoc Account",
